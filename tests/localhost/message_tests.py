...
def test_get_all_comments():
    ...
    result = get('http://localhost:8080/api/messages', headers=apikey).json()
    print(result)
...
def test_get_comment():
    ...
    result = get('http://localhost:8080/api/messages/1', headers=apikey).json()
    print(result)
...
def test_get_wrong_id():
    ...
    result = get('http://localhost:8080/api/messages/9999', headers=apikey).json()
    print(result)
...
def test_edit():
    ...
    result = put('http://localhost:8080/api/messages/2', json=
    {
...
def test_edit_wrong_id():
    # Некорректный запрос: комментария с id = 9999 нет в базе
    result = put('http://localhost:8080/api/messages/9999', json=
    {
...
def test_edit_empty_json():
    ...
    result = put('http://localhost:8080/api/messages/9999', json={}, headers=apikey).json()
    print(result)
    assert result['message'] == 'Message 9999 not found'
...
def test_create():
    ...
    result = post('http://localhost:8080/api/messages', json=
    {
...
def test_delete():
    ...
    max_id = max(get('http://localhost:8080/api/messages', headers=apikey).json()['message'],
                 key=lambda x: x['id'])['id']
    # Удаление комментария
    result = delete(f'http://localhost:8080/api/messages/{max_id}', headers=apikey).json()
    print(result)
    assert result['success'] == 'OK'
...
def test_delete_wrong_id():
    # Некорректный запрос: комментария с id = 9999 нет в базе
    result = delete('http://localhost:8080/api/messages/9999', headers=apikey).json()
    print(result)
    assert result['message'] == 'Message 9999 not found'
